#!/usr/bin/env bash
# ============================================================
# Spring Boot Autoconfiguration Optimizer - Gradle Benchmark Runner
# ============================================================
# Usage: ./benchmarks/scripts/run-benchmarks-gradle.sh [petclinic-jar-path]
#
# This script:
...
echo "=================================================="
echo " Spring Boot Autoconfiguration Optimizer Benchmarks (Gradle)"
echo "=================================================="
echo ""
...
if [ -z "${PETCLINIC_JAR_PATH}" ]; then
    echo "ERROR: PetClinic JAR not found. Build first with: cd integration-tests/petclinic-sample-gradle && ./gradlew bootJar"
    exit 1
fi
...
cat > "${REPORT_FILE}" << EOF
# Spring Boot Autoconfiguration Optimizer - Gradle Benchmark Results
...
\`\`\`bash
# Install Maven core to local repository
mvn install -DskipTests -q -N && mvn install -DskipTests -q -pl autoconfiguration-optimizer-core

# Publish Gradle plugin to local Maven repository
cd spring-boot-autoconfiguration-optimizer-gradle-plugin && ./gradlew --no-daemon publishToMavenLocal && cd ..

# Build the Gradle petclinic sample
cd integration-tests/petclinic-sample-gradle && ./gradlew bootJar && cd ../..

# Run benchmarks
./benchmarks/scripts/run-benchmarks-gradle.sh integration-tests/petclinic-sample-gradle/build/libs/*.jar
\`\`\`
