...
class OptimizedAutoConfigurationEnvironmentPostProcessorTest {
	...
	private final OptimizedAutoConfigurationEnvironmentPostProcessor processor = new OptimizedAutoConfigurationEnvironmentPostProcessor();

	@Test
	void parseConfigurationList_parsesCommaSeparatedList() {
		Set<String> result = processor
			.parseConfigurationList("com.example.FooAutoConfiguration, com.example.BarAutoConfiguration");
		assertThat(result).contains("com.example.FooAutoConfiguration")
	...
	void parseConfigurationList_handlesWhitespaceAroundEntries() {
		Set<String> result = processor
			.parseConfigurationList("  com.example.FooAutoConfiguration  ,  com.example.BarAutoConfiguration  ");
		assertThat(result).contains("com.example.FooAutoConfiguration").contains("com.example.BarAutoConfiguration");
	}
	...
	void applyExclusions_setsExcludeProperty() {
		...
		List<String> toExclude = List.of("com.example.UnusedAutoConfiguration",
				"com.example.AnotherUnusedAutoConfiguration");

		processor.applyExclusions(environment, toExclude);

		String exclusions = environment
			.getProperty(OptimizedAutoConfigurationEnvironmentPostProcessor.EXCLUDE_PROPERTY);
